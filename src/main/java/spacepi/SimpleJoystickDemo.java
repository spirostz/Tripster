...
class SimpleJoystick extends JPanel {
	...
	private void mouseCheck(final MouseEvent e) {
		...
		if (isMouseTracking) {
			...
			curJoySize = (float) Point.distance(mouseX, mouseY, joyCenterX,
					joyCenterY);
		} else {
			curJoySize = 0;
		}
		...
		position.x = (int) (joyOutputRange
				* (Math.cos(curJoyAngle) * curJoySize) / joySize);
	...
	protected void paintComponent(final Graphics g) {
		...
		g2.fillOval((int) (joyCenterX - joyWidth / 2),
				(int) (joyCenterY - joyHeight / 2), (int) joyWidth,
				(int) joyHeight);
