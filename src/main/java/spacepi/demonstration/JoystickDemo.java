...
public class JoystickDemo extends JFrame {
	...
	public static void main(final String[] args) {
		Runnable gui = new Runnable() {
			public void run() {
				new JoystickDemo().setVisible(true);
			}
		};
...
class SimpleJoystick extends JPanel {
	...
	private void mouseCheck(final MouseEvent e) {
		...
		position.x = (int) (joyOutputRange * (Math.cos(curJoyAngle) * curJoySize) / joySize);
		position.y = (int) (joyOutputRange * (-(Math.sin(curJoyAngle) * curJoySize) / joySize));
	...
	protected void paintComponent(final Graphics g) {
		...
		g2.fillOval((int) (joyCenterX - joyWidth / 2), (int) (joyCenterY - joyHeight / 2), (int) joyWidth,
				(int) joyHeight);
	...
	Point positionUpdated(Point position) {
		setSpeedToMotors(position.x, position.y);
		return position;
	}

	void setSpeedToMotors(int x, int y) {
